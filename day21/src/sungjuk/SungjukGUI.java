...
public class SungjukGUI extends JFrame implements ActionListener{
	...
	private JButton save_bt = new JButton("저장");
	private JButton load_bt = new JButton("불러오기");
	...
	*/
	...
	public void actionPerformed(ActionEvent e) {
		if (insert_bt == e.getSource()) {
			...
			pro.insert(st);
			setView();
		}else if (save_bt == e.getSource()) { // proImpl에서 예외처리 해준 경우(강사님)
			pro.save();
			String msg = "파일에 저장이 되었습니다.";
			JOptionPane.showMessageDialog(this, msg, "알림", JOptionPane.INFORMATION_MESSAGE);
			
		}else if (load_bt == e.getSource()) { // 예외전가만 하다가 결국 여기까지 책임전가한 경우 (나)
			try {
				pro.load();
